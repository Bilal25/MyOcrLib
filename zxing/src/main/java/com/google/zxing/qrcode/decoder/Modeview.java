...
public enum Modeview {
  ...
  Modeview(int[] characterCountBitsForVersions, int bits) {
   ...
   */
  public static Modeview forBits(int bits) {
    switch (bits) {
      case 0x0:
        return TERMINATOR;
      case 0x1:
  ...
  /**
   * @param versionv versionv in question
   * @return number of bits used, in this QR Code symbol {@link Versionv}, to encode the
   *         count of characters that will follow encoded in this Mode
   */
  public int getCharacterCountBits(Versionv versionv) {
    int number = versionv.getVersionNumber();
    int offset;
    if (number <= 9) {
      offset = 0;
    } else if (number <= 26) {
