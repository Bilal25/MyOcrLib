 ...
 * @author Sean Owen
 */
public class FinderPatternFinderview {
  ...
  private final List<FinderPatternview> possibleCenters;
  private boolean hasSkipped;
  private final int[] crossCheckStateCount;
  private final ResultPointCallbackview resultPointCallback;
   ...
   */
  public FinderPatternFinderview(BitMatrixview image) {
    this(image, null);
  }

  public FinderPatternFinderview(BitMatrixview image, ResultPointCallbackview resultPointCallback) {
    this.image = image;
  ...
  protected final List<FinderPatternview> getPossibleCenters() {
    return possibleCenters;
  }

  final FinderPatternInfoview find(Map<DecodeHintTypeview,?> hints) throws NotFoundExceptionview {
    ...
    FinderPatternview[] patternInfo = selectBestPatterns();
    ResultPointview.orderBestPatterns(patternInfo);

    return new FinderPatternInfoview(patternInfo);
  }
  ...
  private float crossCheckVertical(int startI, int centerJ, int maxCount,
    ...
    BitMatrixview image = this.image;
    ...
    if (!Float.isNaN(centerI)) {
      ...
      if (!Float.isNaN(centerJ) &&
        ...
        for (int index = 0; index < possibleCenters.size(); index++) {
          FinderPatternview center = possibleCenters.get(index);
        ...
        if (!found) {
          FinderPatternview point = new FinderPatternview(centerJ, centerI, estimatedModuleSize);
  ...
  private int findRowSkip() {
    ...
    ResultPointview firstConfirmedCenter = null;
    for (FinderPatternview center : possibleCenters) {
  ...
  private boolean haveMultiplyConfirmedCenters() {
    ...
    int max = possibleCenters.size();
    for (FinderPatternview pattern : possibleCenters) {
   ...
   */
  private FinderPatternview[] selectBestPatterns() throws NotFoundExceptionview {
    ...
    if (startSize > 3) {
      ...
      for (int i = 0; i < possibleCenters.size() && possibleCenters.size() > 3; i++) {
        FinderPatternview pattern = possibleCenters.get(i);
    ...
    if (possibleCenters.size() > 3) {
      ...
      float totalModuleSize = 0.0f;
      for (FinderPatternview possibleCenter : possibleCenters) {
   ...
   */
  private static final class FurthestFromAverageComparator implements Comparator<FinderPatternview>, Serializable {
  ...
  /**
   * <p>Orders by {@link FinderPatternview#getCount()}, descending.</p>
   */
  private static final class CenterComparator implements Comparator<FinderPatternview>, Serializable {
    private final float average;
