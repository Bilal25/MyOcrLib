...
/**
 * This object renders a CODE39 code as a {@link BitMatrixview}.
 *
 * @author erik.barbara@gmail.com (Erik Barbara)
 */
public final class Code39Writerview extends OneDimensionalCodeWriterview {
  ...
  public BitMatrixview encode(String contents,
    ...
    if (format != BarcodeFormatview.CODE_39) {
  ...
  public boolean[] encode(String contents) {
    ...
    for (int i = 0; i < length; i++) {
      int indexInString = Code39Readerview.ALPHABET_STRING.indexOf(contents.charAt(i));
      if (indexInString < 0) {
        throw new IllegalArgumentException("Bad contents: " + contents);
      }
      toIntArray(Code39Readerview.CHARACTER_ENCODINGS[indexInString], widths);
      for (int width : widths) {
        codeWidth += width;
      }
    }
    boolean[] result = new boolean[codeWidth];
    toIntArray(Code39Readerview.ASTERISK_ENCODING, widths);
