...
/**
 * This object renders a CODE93 code as a BitMatrixview
 */
public class Code93Writerview extends OneDimensionalCodeWriterview {
  @Override
  public BitMatrixview encode(String contents,
    ...
    if (format != BarcodeFormatview.CODE_93) {
  ...
  public boolean[] encode(String contents) {
    ...
    //start character (*)
    toIntArray(Code93Readerview.CHARACTER_ENCODINGS[47], widths);

    boolean[] result = new boolean[codeWidth];
    int pos = appendPattern(result, 0, widths);

    for (int i = 0; i < length; i++) {
      int indexInString = Code93Readerview.ALPHABET_STRING.indexOf(contents.charAt(i));
      toIntArray(Code93Readerview.CHARACTER_ENCODINGS[indexInString], widths);
      pos += appendPattern(result, pos, widths);
    }

    //add two checksums
    int check1 = computeChecksumIndex(contents, 20);
    toIntArray(Code93Readerview.CHARACTER_ENCODINGS[check1], widths);
    pos += appendPattern(result, pos, widths);

    //append the contents to reflect the first checksum added
    contents += Code93Readerview.ALPHABET_STRING.charAt(check1);

    int check2 = computeChecksumIndex(contents, 15);
    toIntArray(Code93Readerview.CHARACTER_ENCODINGS[check2], widths);
    pos += appendPattern(result, pos, widths);
